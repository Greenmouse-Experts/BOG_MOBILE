...
class _CheckoutState extends State<Checkout> {
 ...
 checkout({required int cost, required String email,required HomeController controller}) async {
        var logInDetails = LogInModel.fromJson(jsonDecode(MyPref.logInDetail.val));
        int price = cost * 100;
           final List<OrderProduct> orderProducts =[];
          for (var item in controller.cartItems.values.toList()){
               orderProducts.add(OrderProduct(productId: item.product.id!, quantity: item.quantity));
          }
          final shippingAddress = ShippingAddress(city: city.text, state: state.text, country: country.text, postalCode: zip.text, address: address.text, contactName: '${logInDetails.fname} ${logInDetails.lname}', contactPhone: logInDetails.phone!, contactEmail: logInDetails.email!);
        ...
        if (response.status == true) {
          ...
          final postOrder = PostOrder(products: orderProducts, shippingAddress: shippingAddress, paymentInfo: PaymentInfo(reference: response.reference!, amount: cost), discount: 0, deliveryFee: 5000, totalAmount: cost + 5000).toJson();
      
        final respose =  await controller.userRepo.postData('/orders/submit-order', postOrder);
         print(respose.data);
          Get.to(() => AppReceipt( message: successMessage));
        } else {
