                ...
                child: Column(
                  children: [
                    PageInput(
                      ...
                      validator: MinLengthValidator(5, errorText: 'Enter a valid work name'),
                    ),
                    ...
                    PageInput(
                      ...
                      validator: MinLengthValidator(1, errorText: 'Enter a valid amount'),
                    ),
                    const SizedBox(height: 8),
                    AppDatePicker(
                 ...
                 //   ElevatedButton(onPressed: (){}, child: Text('View Document')),
                    const SizedBox(height: 12),
                    PageInput(
                      hint: '',
                      ...
                      keyboardType: TextInputType.number,
                      validator: MinLengthValidator(1,errorText: 'Enter a valid number of years'),
                    ),
                    const SizedBox(height: 8),
                    PageInput(
                        hint: '',
                    ...
                    const SizedBox(height: 15),
                     if (!widget.isNewWork)
                    AppButton(
                      title: 'View Document', onPressed: (){
                           if (widget.workExperience!.fileUrl!.endsWith('.jpg') || widget.workExperience!.fileUrl!.endsWith('.png')|| widget.workExperience!.fileUrl!.endsWith('.jpeg')){
                              Get.to(()=> PhotoViewPage(url: widget.workExperience!.fileUrl!));
                           } else if (widget.workExperience!.fileUrl!.endsWith('.pdf')) {
                            Get.to(() => PdfViewerPage(path: widget.workExperience!.fileUrl!));
                           } else {
                            Get.snackbar('Error', 'File type not supported currently', backgroundColor: Colors.red);
                           }
                      },),
                      if (widget.isNewWork)
                    AppButton(title: 'Submit', onPressed:  () async {
                      if (widget.isNewWork){
                              if (pickedFile == null){
                        
                         Get.snackbar('No File Picked', 'Pick a File', backgroundColor: Colors.red);
                         return;
                      }
                    print(pickedFile!.path);
                     if (_formKey.currentState!.validate()) {
                        final newWorkExperience = {
                          ...
                          "userType" : userType,
                          "document" : [await  dio.MultipartFile.fromFile(pickedFile!.path, filename: pickedFile!.path
                                                      .split('/')
                                                      .last)]
                        };
      
                        var formData = dio.FormData.fromMap(newWorkExperience);
                          ...
                          if (response.isSuccessful) {
                                      Get.back();
                                      Get.snackbar('Success', 'Work Experience Updated Successfully', backgroundColor: Colors.green);
                                    } else {
